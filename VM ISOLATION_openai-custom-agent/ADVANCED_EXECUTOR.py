...
def get_query_context(openai_client, user_message, model):
    ...
    system_message = ADVANCED_PROMPT_MANAGEMENT.SYSTEM_PROMPT_TOOL_SELECTION

    response = openai_client.chat.completions.create(
        model=model,
        messages=[system_message, user_message],
        tools=ADVANCED_PROMPT_MANAGEMENT.TOOLS,
...
def query_log_analytics(log_analytics_client, workspace_id, timerange_hours, table_name, account_name, device_name, fields, caller, user_principal_name, start_time, end_time):
    if start_time and end_time:
        time_filter = (
            f'| where TimeGenerated between (datetime("{start_time}")..datetime("{end_time}"))'
        )
    elif start_time:
        time_filter = f'| where TimeGenerated >= datetime("{start_time}")'
    elif end_time:
        time_filter = f'| where TimeGenerated <= datetime("{end_time}")'
